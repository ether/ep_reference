# Contributing to ep_reference

Thanks for helping improve `ep_reference` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_reference` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_reference from your local clone
pnpm run plugins i --path ../ep_reference

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_reference/` are picked up after a server restart.
...
* Etherpad version, ep_reference version, browser + OS
* Steps to reproduce
